...
cold exhaust — and the wall temperature goes *up* by 241 °C before it comes
down."""))
...
def P(sym, case="both"):
    ...
    sel = par[(par.symbol == sym) & (par.case.isin([case, "both"]))]
...
def d_slattery_bird(sp, T, p_atm=1.0, b_sp="N2"):
    """Binary diffusivity in cm2/s, non-polar pair (Bird et al. 1960, eq. 16.3-1)."""
    a, b = 2.745e-4, 1.823
    tc = TC[sp] * TC[b_sp]
    return (a * (T / np.sqrt(tc)) ** b * (PC[sp] * PC[b_sp]) ** (1 / 3)
            * tc ** (5 / 12) * np.sqrt(1 / MW[sp] + 1 / MW[b_sp]) / p_atm)


def lam_gas(T):
    """Thermal conductivity of the mixture, approximated by N2 (paper, p. 31)."""
    return P("lambda_g_A") * T ** P("lambda_g_n")


def cp_solid(T):
    """Table I: C_ps(T), J/(g K)."""
    return P("C_ps_c0") + P("C_ps_c1") * T + P("C_ps_c2") / T ** 2'''))
...
cells.append(code('''class Monolith:
    ...
    def _build_scaling(self, shape):
        ...
        km_ref = (self.sh * np.array([d_slattery_bird(s, 600.0) for s in SPECIES])
    ...
    def rates(self, y_s, T_s):
        ...
        k1 = P("k1_A") * np.exp(-P("k1_E_over_R") / T_s)
        k2 = P("k2_A") * np.exp(-P("k2_E_over_R") / T_s)
        k3 = P("k3_A") * np.exp(-P("k3_E_over_R") / T_s)
        K1 = P("K1_A") * np.exp(-P("K1_dH_over_R") / T_s)
        K2 = P("K2_A") * np.exp(-P("K2_dH_over_R") / T_s)
        K3 = P("K3_A") * np.exp(-P("K3_dH_over_R") / T_s)
        K4 = P("K4_A") * np.exp(-P("K4_dH_over_R") / T_s)
    ...
    def transport(self, Tg):
        ...
        d_i = np.stack([d_slattery_bird(s, Tg) for s in SPECIES], axis=-1)
    ...
    def enthalpy(self, T):
        ...
        return P("C_ps_c0") * T + 0.5 * P("C_ps_c1") * T ** 2 - P("C_ps_c2") / T
...
cells.append(code('''def case2(lam_s=None, n_x=160, dt=0.05, t_end=16.0, nu=3.608,
          enthalpy_form=True):
...
cells.append(md(r"""### 1. Conservation, to machine precision

Two balances that the discretisation should satisfy exactly, and will not if the
divergence and convection operators are paired wrongly or a boundary term is
dropped.
...
cells.append(code('''# species balance at every stored time of the case-1 run
sp_err = []
for s in snap1:
    supplied = m1.g_mol * (y1 - s["yg"][-1])
    reacted = s["rate_int"]
    scale = max(abs(supplied[0]), 1e-30)
    sp_err.append(np.abs(supplied[:4] - reacted[:4]).max() / scale)
...
en_closure = float(np.abs(en_res).sum() / np.abs(dts * q_in).sum())

print(f"species balance, worst relative closure over 3001 time levels : {sp_closure:.2e}")
print(f"energy balance, integrated relative closure over 300 s        : {en_closure:.2e}")'''))

cells.append(md(r"""### 2. The steady state must reach the adiabatic temperature the conversion pays for

At long times the solid stops storing heat, so the gas has to carry away exactly
what the reaction releases. That turns into a statement about temperatures with
no free parameter: the outlet gas rise equals
$\sum_i(-\Delta H)_i\,\Delta c_{g,i}/(M C_{pg})$.

And then the check the paper actually asserts: with H$_2$ present the *wall* at
the inlet ends up **above** that adiabatic temperature, because H$_2$ has a Lewis
number near 4 and diffuses to the wall faster than the heat it liberates can be
conducted away."""))

cells.append(code('''dy = y1 - snap1[-1]["yg"][-1]
...
for nu, label in [(2.976, "Nu=Sh=2.976  (const. wall temperature, T)"),
    ...
    (Tm, xm, tm), _, _ = case2(nu=nu, n_x=80, t_end=16.0)
    alts.append(dict(variant=label, T_max=Tm, x_max=xm, t_peak=tm))
(Tm, xm, tm), _, _ = case2(n_x=80, t_end=16.0, enthalpy_form=False)
...
for n_x, dt in [(40, 0.1), (80, 0.2), (80, 0.1), (80, 0.05), (160, 0.1)]:
    ...
    tt, ss = mm.run(60.0, dt, record=lambda s: dict(X=1.0 - s.u[-1, 2:7] / s.y_in))
...
for n_x, dt in [(40, 0.05), (80, 0.05), (160, 0.1), (160, 0.05), (160, 0.025), (320, 0.05)]:
    (Tm, xm, tm), _, _ = case2(n_x=n_x, dt=dt, t_end=16.0)
...
report_agreement("I1.2", {
    ...
    "energy_balance_closure": en_closure,
...
1. **The refinement study above is affordable.** Fifteen runs at four grids and
   four time steps take under two minutes. In 1982 that was the reason to fix
   $N = 81$ and not ask the question — and the paper's own $x_{\max}/L$ values
   carry the fingerprint of that grid.
