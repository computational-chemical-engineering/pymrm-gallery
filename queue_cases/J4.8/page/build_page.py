...
categories: [sec:J, struct:S1, tier:T1, data:tier2, phase:liquid-biofilm]
date: 2026-07-30
---
...
the task group's 1983 remit was to find "the simplest [model] having the
capability of realistic predictions".
...
stoi  = load_data("henze-1987-table1-stoichiometry.csv", page=PAGE)
...
def lane(nu_m=None, n=150, f_anox=0.30, c_feed=None, p=None, R=R_INT,
    ...
    z_f = np.linspace(0.0, 1.0, n + 1)                # dimensionless lane coordinate
    z_c = 0.5 * (z_f[:-1] + z_f[1:])
    ...
    otr = float(np.sum(kla * (SO_SET - c[:, IX["S_O"]])) / n)
    return dict(c=c, c_in=c_in, z=z_c, n=n, u=u, otr=otr, outer=it + 1,
...
cells.append(code(r'''def show_matrix(nu, comps, title):
    print(title)
    print("   " + "".join(f"{c:>17s}" for c in comps))
    for j in sorted(nu):
        row = "".join(f"{('.' if nu[j][c] == 0 else sp.sstr(nu[j][c])):>17s}" for c in comps)
        print(f"{j}  {row}")
    print()


show_matrix(NU, C13[:7], "stoichiometric matrix, components 1-7 (COD units)")
show_matrix(NU, C13[7:] + ["S_N2"], "components 8-13 (+ the added S_N2)")
for r in rates.itertuples():
    print(f"rho_{r.j} = {r.rate}")'''))
...
print("   is that two irrational-looking fractions were printed to two decimals.")'''))
...
print(f"   {'j':>2} {'cell':<7}{'law':<8}{'recovered':>32}{'printed':>32}   verdict")
for j, cell, law in targets:
    ...
    printed = NU[j][cell]
    diff = sp.simplify(sol - printed)
    if diff == 0:
        verdict, n_exact = "identical", n_exact + 1
    else:
        verdict, n_rounding = f"differs by {sp.nsimplify(diff)}", n_rounding + 1
    print(f"   {j:>2} {cell:<7}{law:<8}{sp.sstr(sp.simplify(sol)):>32}"
          f"{sp.sstr(printed):>32}   {verdict}")
print(f"\n   {n_exact} of {len(targets)} are symbolically identical to the printed cell.")
print(f"   The other {n_rounding} differ only by the 2.86 / 4.57 rounding of check 1.")'''))
...
for tag, M in (("printed 2.86 / 4.57", NU_M), ("exact 20/7 / 32/7", NU_M_EXACT)):
    for n in (75, 150, 300):
        s = lane(nu_m=M, n=n)
        r_int = rho(s["c"], P).sum(axis=0) / n       # integral of each rate over the lane
...
print(f"\n   Nitrogen and charge close to {max(abs(b['N']) for b, _, _ in rows.values()):.0e}"
      f" and {max(abs(b['charge']) for b, _, _ in rows.values()):.0e} g m-3 d-1 -"
      " machine precision on")
print("   an oxygen demand of "
      f"{s150['otr']:.0f} g m-3 d-1. That is a real test of the discretisation:")
print("   the upwind fluxes and the source term must cancel exactly, and they do.")
print(f"\n   COD does not close. It leaks {b150['COD']:.4f} g O2 m-3 d-1, which the")
print(f"   matrix's own residuals predict to {abs(b150['COD'] - pred150) / abs(pred150):.1e}"
      " relative - so the leak is")
print("   stoichiometric, not numerical. It is also grid independent, which numerical")
print("   error would not be.")
print(f"\n   Swap in 20/7 and 32/7 and the same lane closes to {abs(bx150['COD']):.1e},")
print(f"   a factor of {abs(b150['COD'] / bx150['COD']):.0e} better, with nothing else changed.")
print(f"   In practical terms the printed rounding mis-states the oxygen requirement")
print(f"   by {100 * abs(b150['COD']) / s150['otr']:.3f} % - negligible for design, and")
print("   decisive as evidence that the transcription is right.")'''))

cells.append(code(r'''print("5. GRID CONVERGENCE OF THE LANE\n")
ref = lane(n=1200)
print(f"   {'n':>6}{'S_NH out':>12}{'S_NO out':>12}{'alk out':>12}{'max rel err':>14}{'ratio':>8}")
prev = None
for n in (75, 150, 300, 600):
    s = lane(n=n)
    e = float(np.max(np.abs(s["c"][-1] - ref["c"][-1]) / np.maximum(np.abs(ref["c"][-1]), 1e-3)))
    ratio = "" if prev is None else f"{prev / e:8.2f}"
    print(f"   {n:>6}{s['c'][-1, IX['S_NH']]:12.5f}{s['c'][-1, IX['S_NO']]:12.5f}"
          f"{s['c'][-1, IX['S_ALK']] * 50:12.4f}{e:14.3e}{ratio}")
    prev = e
print("\n   Ratios approach 2: first order, which is what upwind convection gives.")
...
optional. It says of alkalinity that "its inclusion is not essential, but it is
...
It can be asked directly. Two coefficients in the matrix settle the arithmetic
before any solver runs: nitrification takes $1/7$ mol of alkalinity per gram of N
oxidised and denitrification returns $1/14$, so **denitrification recovers
exactly half of what nitrification destroys** — 3.57 against 7.14 g CaCO₃ per g N.
Everything below is that identity, put through a reactor with real switching
functions and a real recycle."""))

cells.append(code(r'''ALK_N_nit = float(-(NU[3]["S_ALK"] / NU[3]["S_NO"]).subs(STOICH)) * 50.0
ALK_N_den = float((NU[2]["S_ALK"] / -NU[2]["S_NO"]).subs(STOICH)) * 50.0
print(f"from the matrix alone: nitrification destroys {ALK_N_nit:.3f} g CaCO3 per g N,")
print(f"                       denitrification returns {ALK_N_den:.3f} g CaCO3 per g N")
print(f"                       ratio {ALK_N_nit / ALK_N_den:.4f}")
print("(the small departure from exactly 2 is the ammonium assimilated into biomass,")
print(" the -i_XB/14 term that both rows carry)\n")
...
need = 6.0 - (ALK - 50.0) / 25.0        # raw-water alkalinity needed to hold the limit
...
print(f"And the alkalinity column earns its place. With no unaerated volume this lane")
print(f"needs {need[0]:.2f} mol m-3 of raw-water alkalinity to stay above the paper's")
print(f"threshold; a 30 % anoxic zone drops that to {need[np.argmin(abs(f_grid - 0.30))]:.2f},")
print(f"and a 50 % one to {need[np.argmin(abs(f_grid - 0.50))]:.2f}. Half the soft waters in Europe sit")
print("between those numbers, which is the whole reason the column is in the table.")'''))
...
for label, ka in (("k_a = 0.016 (the row carrying the model's symbol)", 0.016),
    ...
    print(f"   {label:<48} S_ND {s[IX['S_ND']]:6.3f}  S_NH {s[IX['S_NH']]:6.3f}"
...
without it, and the missing 0.17 g N per g of biomass grown anoxically is easy to
mistake for a bug in your own code.
