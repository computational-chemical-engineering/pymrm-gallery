...
1. **Traceability.** Every number on the page must come from a printed source, a
   ...
   cannot trace is a finding.
2. **Circularity.** Is any "agreement" using a quantity that was fitted to the
   thing it is being compared against? Was a constant tuned until the comparison
   worked?
3. **Confounding.** For any group comparison, do the groups overlap in every
   other variable? If not, the difference may be the confound.
4. **Resolving power.** Is the claimed effect larger than the figure's line width
   or the digitisation error? If not, agreement is not evidence.
5. **Direction and convention.** Deviations defined consistently? Boundary
   conditions on the outward normal? Reciprocals at >5 % scatter are not
   interchangeable.
6. **The honesty of the caveats.** Does the page overstate? Does it call
